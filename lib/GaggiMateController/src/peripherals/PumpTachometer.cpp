...
uint32_t captureTicksToUs(uint32_t ticks) {
    ...
    return static_cast<uint32_t>((static_cast<uint64_t>(ticks) * 1000000ULL + 40000000ULL) / 80000000ULL);

}

uint32_t computeHoldoffUs(const PumpTachometer::Config &config, uint32_t lastPeriodUs) {
    if (lastPeriodUs == 0) {
        return config.holdoffMinUs;
    const uint32_t divisor = std::max<uint32_t>(static_cast<uint32_t>(config.holdoffPeriodDivisor), 1U);
    return std::clamp(lastPeriodUs / divisor, config.holdoffMinUs, config.holdoffMaxUs);
}
...
bool PumpTachometer::begin(const Config &config) {
    ...
    if (gpio_install_isr_service(0) != ESP_OK && gpio_install_isr_service(ESP_INTR_FLAG_IRAM) != ESP_OK) {
        _enabled = false;
        return false;
    }
...
bool IRAM_ATTR PumpTachometer::captureThunk(mcpwm_unit_t, mcpwm_capture_channel_id_t, const cap_event_data_t *edata, void *userData) {
...
void PumpTachometer::update() {
    ...
    if (_lastWindowUpdateUs == 0) {
        ...
        if (windowElapsedUs >= static_cast<int64_t>(_config.countWindowUs)) {
            ...
            if (_pcntEnabled && deltaPulsesAccepted >= _config.minCountWindowPulses && deltaPulsesPcntRaw > 0) {

                _sample.pcntRatio = static_cast<float>(deltaPulsesPcntRaw) / static_cast<float>(std::max<uint32_t>(deltaPulsesAccepted, 1U));

                pcntWindowHealthy = (_sample.pcntRatio >= _config.pcntLegacyMinRatio) &&
            ...
            if (_pcntEnabled) {
                ...
                if (pcntWindowHealthy) {
                    ...
                    _pcntGoodWindows = static_cast<uint8_t>(std::min<uint32_t>(_pcntGoodWindows + 1U, 255U));
                    ...
                    if (deltaPulsesPcntRaw > 0 && deltaPulsesAccepted >= _config.minCountWindowPulses) {

                        _softwareRejects++;

                    }

                    _pcntBadWindows = static_cast<uint8_t>(std::min<uint32_t>(_pcntBadWindows + 1U, 255U));
