...
uint32_t computeHoldoffUs(const PumpTachometer::Config &config, uint32_t lastPeriodUs) {
    if (lastPeriodUs == 0) {
        return config.minPeriodUs;
    }
    return std::min(config.minPeriodUs, lastPeriodUs);
}
...
void IRAM_ATTR PumpTachometer::onCaptureIsr(uint32_t captureValue, uint32_t captureEdge) {
    ...
    if (_captureEnabled && captureEdge != MCPWM_NEG_EDGE) {
    ...
    if (_lastWindowUpdateUs == 0) {
        ...
        if (windowElapsedUs >= static_cast<int64_t>(_config.countWindowUs)) {
            ...
            if (deltaPulsesAccepted >= _config.minCountWindowPulses) {
                _sample.rpmCountWindow = (static_cast<float>(deltaPulsesAccepted) * 60000000.0f) /
            ...
            if (deltaPulsesPcntRaw >= _config.minCountWindowPulses) {
                _sample.rpmCountWindowPcntRaw = (static_cast<float>(deltaPulsesPcntRaw) * 60000000.0f) /
                                                (_config.pulsesPerRevolution * static_cast<float>(windowElapsedUs));
            ...
            if (_pcntEnabled && deltaPulsesAccepted >= _config.minCountWindowPulses && deltaPulsesPcntRaw > 0) {
                _sample.pcntRatio = static_cast<float>(deltaPulsesPcntRaw) /
