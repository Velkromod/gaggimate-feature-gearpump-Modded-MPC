...
MCP4725::MCP4725(const uint8_t deviceAddress, SoftWire *wire) {
...
uint8_t MCP4725::getAddress() { return _deviceAddress; }
...
uint16_t MCP4725::getValue() { return _lastValue; }
...
float MCP4725::getPercentage() { return getValue() * (100.0 / MCP4725_MAXVALUE); }

void MCP4725::setMaxVoltage(float v) { _maxVoltage = v; }

float MCP4725::getMaxVoltage() { return _maxVoltage; }

int MCP4725::setVoltage(float v) { return setValue(round((v * MCP4725_MAXVALUE) / _maxVoltage)); }

float MCP4725::getVoltage() { return getValue() * (_maxVoltage / MCP4725_MAXVALUE); }
...
uint32_t MCP4725::getLastWriteEEPROM() { return _lastWriteEEPROM; };
